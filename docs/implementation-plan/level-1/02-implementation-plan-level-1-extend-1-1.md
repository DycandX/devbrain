# Implementation Plan - Level 1 Extend (v1.1 - v1.2): Graph Mesh, Workspace Harvester & Targeted Ingestion
...
Ekstensi **Level 1.1 - 1.2** bertujuan untuk menyelesaikan **kebutuhan lanjutan manajemen repositori & graf pengetahuan**:
...
2. **Workspace Project Harvester & Targeted Single Ingest:**
   * Batch scan seluruh folder projek (`devbrain ingest projects`).
   * Targeted single repo ingest (`devbrain ingest project <path>`) saat pengguna baru membuat projek atau baru meng-clone repo.
3. **Auto-Inspector & Klasifikasi Taksonomi Cerdas:**
   * Mendeteksi otomatis ragam repositori: **Active Projects** (`10_Projects/`), **Cloned Code Reference** (`20_Knowledge/External_Repos/`), **Agent Skills Mesh** (`00_System/Agent_Skills/`), dan **Knowledge / Markdown Docs** (`20_Knowledge/References/`).
...
│   ├── inspector.py              # [NEW Sprint 06] Auto-Inspector jenis repo (Skill vs Docs vs Project vs Reference)
...
└── cli/
    └── commands/
        └── ingest_cmd.py         # [Updated Sprint 06] Dukungan `ingest`, `ingest project`, `ingest projects`, & `ingest all`
```
...
graph TD
    ...
    subgraph "Sprint 06 (v1.2.0-alpha - RENCANA BERIKUTNYA)"
        S06_T1["Task 01: Auto-Inspector, Repo Scanner & Manifest Parser"]
        S06_T2["Task 02: Auto-Seeding Multi-Type Cards (Projects, Knowledge, Skills)"]
        S06_T3["Task 03: Auto-Entity Linker & Backlink Injector"]
        S06_T4["Task 04: CLI `devbrain ingest project`, `projects` & `all`"]
...
* **Target File:** `src/devbrain/harvester/inspector.py`, `src/devbrain/harvester/manifest_parser.py`, `src/devbrain/harvester/project_harvester.py`
* **Deskripsi:**
  * Memindai direktori target atau folder `workspace_roots`.
  * **Auto-Inspector:** Mengidentifikasi apakah repo berupa **Agent Skill** (`SKILL.md`), **Knowledge Docs** (mayoritas `.md`), **Active Project** (source code + author lokal), atau **External Reference** (source code + author luar).
  * Mengurai manifest dependensi: Python (`pyproject.toml`, `requirements.txt`), Node/TS (`package.json`), Rust (`Cargo.toml`), Go (`go.mod`), DevOps (`Dockerfile`, `docker-compose.yml`).

### 📄 Task 02: Auto-Seeding Multi-Type Cards (`10_Projects/`, `20_Knowledge/`, `00_System/Agent_Skills/`)
* **Target File:** `src/devbrain/harvester/project_harvester.py`, `src/devbrain/core/scaffolder.py`
* **Deskripsi:**
  * **Projek Internal:** Auto-seed kartu di `10_Projects/<Nama_Projek>/README.md` (`type: "project"`, `role: "owner" | "contributor"`).
  * **Projek Referensi:** Auto-seed kartu di `20_Knowledge/External_Repos/<Nama_Repo>/README.md` (`type: "reference-repo"`, `role: "study"`).
  * **Agent Skill:** Mengimpor folder skill ke `00_System/Agent_Skills/<skill-name>/` sehingga langsung aktif bagi seluruh agen.
  * **Knowledge Docs:** Menyalin dan mengindeks file markdown ke `20_Knowledge/References/<Repo>/`.
...
* **Deskripsi:**
  * `devbrain ingest project <path> [--type <auto|project|skill|knowledge|reference>]`: Ingest 1 repo spesifik secara presisi.
  * `devbrain ingest projects [--dir <path>] [--dry-run]`: Batch scan seluruh workspace root.
  * `devbrain ingest all`: Full scan (seluruh repo + seluruh sesi AI + link seluruh graf).
...
* **Deskripsi:**
  * Unit dan integration tests untuk auto-inspector, manifest parser, targeted single ingestion, dan auto-wikilink generation.
